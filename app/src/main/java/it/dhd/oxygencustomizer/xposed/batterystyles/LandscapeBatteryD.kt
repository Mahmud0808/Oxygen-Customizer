...
open class LandscapeBatteryD(private val context: Context, frameColor: Int) :
    BatteryDrawable() {
    ...
    private fun loadPaths() {
        val pathString =
            getResources(context).getString(R.string.config_landscapeBatteryPerimeterPathD)
        perimeterPath.set(PathParser.createPathFromPathData(pathString))
        perimeterPath.computeBounds(RectF(), true)

        val errorPathString =
            getResources(context).getString(R.string.config_landscapeBatteryErrorPerimeterPathD)
        errorPerimeterPath.set(PathParser.createPathFromPathData(errorPathString))
        errorPerimeterPath.computeBounds(RectF(), true)

        val fillMaskString =
            getResources(context).getString(R.string.config_landscapeBatteryFillMaskD)
        fillMask.set(PathParser.createPathFromPathData(fillMaskString))
        // Set the fill rect so we can calculate the fill properly
        fillMask.computeBounds(fillRect, true)

        val fillOutlinePathString =
            getResources(context).getString(R.string.config_landscapeBatteryFillOutlineD)
        fillOutlinePath.set(PathParser.createPathFromPathData(fillOutlinePathString))
        fillOutlinePath.computeBounds(RectF(), true)

        val boltPathString =
            getResources(context).getString(R.string.config_landscapeBatteryBoltPathD)
        boltPath.set(PathParser.createPathFromPathData(boltPathString))

        val plusPathString =
            getResources(context).getString(R.string.config_landscapeBatteryPowersavePathD)
